...
class Plugin_License implements Registerable, License {
     ...
     * @param License_API The API to perform the various license actions (activate, deactivate, etc).
    ...
    public function is_valid() {
        ...
        return $this->get_license_key() && in_array( $this->get_status(), array( 'active', 'expired' ) );
    }
    ...
    public function get_renewal_url( $apply_discount = true ) {
        ...
        if ( ! empty( $license_info['item_id'] ) ) {
            ...
            return Util::format_store_url( 'our-wordpress-plugins' );
        }
    }
    ...
    private function get_license_expired_message() {
        return __( 'Your license key expired has expired.', 'posts-table-pro' );
    }
