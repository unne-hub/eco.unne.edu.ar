...
class License_Key_Setting implements Registerable, License_Setting {
        ...
        if ( $this->is_license_action( self::ACTIVATE_KEY ) ) {
            ...
            if ( isset( $license_setting['license'] ) ) {
                ...
                $this->add_settings_message(
                    __( 'License key successfully activated.', 'posts-table-pro' ),
                    __( 'There was an error activating your license key.', 'posts-table-pro' ),
                    $activated
                );
            }
            ...
            $this->add_settings_message(
                __( 'License key successfully deactivated.', 'posts-table-pro' ),
                __( 'There was an error deactivating your license key, please try again.', 'posts-table-pro' ),
                $deactivated
            );
            ...
            $this->add_settings_message(
                __( 'The license key looks good!', 'posts-table-pro' ),
                __( 'There\'s a problem with your license key.', 'posts-table-pro' ),
    ...
    public function get_license_key_setting() {
        ...
        if ( $this->is_woocommerce ) {
            $setting['desc_tip'] = __( 'The licence key is contained in your order confirmation email.', 'posts-table-pro' );
        } elseif ( $this->is_edd ) {
            // EDD uses title case for setting names, so let's keep things consistent.
            $setting['title'] = __( 'License Key', 'posts-table-pro' );
    ...
    private function get_license_description() {
        $buttons = array(
            ...
            'activate'   => $this->license_action_button( self::ACTIVATE_KEY, __( 'Activate', 'posts-table-pro' ) ),
            'deactivate' => $this->license_action_button( self::DEACTIVATE_KEY, __( 'Deactivate', 'posts-table-pro' ) )
        );
