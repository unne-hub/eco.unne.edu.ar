...
use Barn2\Plugin\Posts_Table_Pro\Data\Abstract_Table_Data,
    Barn2\Plugin\Posts_Table_Pro\Data\Post_Title,
...
class Post_Custom_Field extends Abstract_Table_Data {
    ...
    public function get_data() {
        ...
        // We exclude ACF date picker fields, as the date formatting for those is handled by get_acf_value, so we don't double up.
        ...
        if ( 0 === strpos( $cf_value, 'http' ) && $url = filter_var( $cf_value, FILTER_VALIDATE_URL ) ) {
            ...
            $cf_value  = sprintf(
                '<a href="%1$s">%2$s</a>',
                apply_filters( 'posts_table_url_custom_field_link', $url, $this->field, $this->post ),
                apply_filters( 'posts_table_url_custom_field_text', $link_text, $this->field, $this->post )
            );
        }
    ...
    private function is_date_field() {
        return in_array( 'cf:' . $this->field, $this->date_columns ) || $this->is_acf_date_field();
    }

    private function is_acf_date_field() {
        return $this->is_acf_field && in_array( $this->acf_field_object['type'], [ 'date_picker', 'date_time_picker', 'time_picker' ] );
    }
    ...
    private function convert_to_timestamp( $date ) {
        ...
        if ( Util::is_european_date_format( $format ) || apply_filters( 'posts_table_custom_field_is_eu_au_date', false, $this->field ) ) {
